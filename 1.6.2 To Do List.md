...
Most important ones: \* Fix model issue where removing any row removes
the last row as well. (midiinputmappingtablemodel.cpp, midimapping.cpp)
\* Fix model issue where completing the learning wizard doesn't list
anything in the table until Mixxx is restarted (probably same as above)
\* Get rid of MidiType enum, switch MIDI mapping format to use
\<status\> for input blocks as well. (Albert probably) \* **Non-coding
task:** Test mono MP3/OGG/FLAC/WAVs \* Multithreading guru task: Clean
up and check MidiScriptEngine thread-safety \*
...
(in no particular order)
  ...
  - Fix up \<status\> and \<miditype\> mishandling in MidiMessage for
    outputs (Albert needs to ask Sean about this). 
