...
``` 
    ...
    * Need a way to keep track of duplicate devices so separate files can be saved:
      - Have DlgPreferences::slotApply() call MidiDeviceManager::saveMappings() which does them all in one shot (and can thereby keep track of them) OR
      - Set up a QMap in MidiDeviceManager (that MidiMapping can access) that maps device names (from MidiDeviceManager::m_devices) to file names. These would be populated on device scan.
