...
**Remaining blocker bugs for 1.8.0 final**
  ...
  - Scaling ASSERTs
  - EngineBufferScaleLinear audio degradation (dropped samples?)
  - EngineBufferScaleST repeated buffers, sync loss
  - ~~"scratch" control is asymmetric -
    <https://bugs.launchpad.net/mixxx/+bug/519892>~~ - fixed in
    fixes\_scratch
  - \<del\>fixes\_scratch merge\<del\> 

**Very important remaining tasks for beta2**:

  - Merge sadness\_ssplugin and ~~build plugins for each platform~~
  - ~~Merge features\_sqlite into trunk~~
  - ~~Merge features\_scriptTimers (with its MIDI thread fixes)~~
  - ~~M4A Plugin + Downloader (done)~~
  - ~~Update LibraryTableModel's rows when TrackDAO is accessed, without
    resetting it.~~ - in features\_sqlite branch
...
### Engine
  ...
  - Implement alpha-beta smoothing filter in the Engine (currently in
    scratch.\* functions in the common script file) with fixed-interval
    updates. (I.e. if no new data arrives in the interval, assume the
    control is stopped and feed that data to the smoothing function.)
  - **xwax code** will be very useful for this. From Mark Hills:

<!-- end list -->

    Well I think the xwax source code is your friend here.
    ...
    Search for pitch_dt_observation() in timecoder.c.
