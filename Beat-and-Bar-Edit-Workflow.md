...
With the bottom number of the time signature together with the BPM, we can determine the beat length in a absolute unit, seconds. The upper number of the time signature restrict how many beats are allowed inside each measure, so after we have computed the beat length we use that to determine the absolute bar length. 
