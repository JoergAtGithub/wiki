...
Skin can change Mixxx\` changes look and feel‚ some skins merely make
the program more aesthetically pleasing while others rearrange elements
of the interface, potentially making the program easier to use.
...
Channel (X=1 or 2), Hotcue # (Y=1-32) & performed action (activate), depends on  # of Hotcues defined , see hotcue_X_position
Action is performed while playing (false) on klick on element
Which mouse button must be clicked so the action is performed
