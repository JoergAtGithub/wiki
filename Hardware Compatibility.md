...
currently in production and suitable for use with Mixxx. All the USB
...
| [Focusrite Scarlett Solo](https://us.focusrite.com/usb-audio-interfaces/scarlett-solo)                                                    | $100               | 2            | 2           | A cheap but quality option suitable for broadcasting with a microphone input using direct monitoring. The headphones cannot play a separate signal from the main output, so the onboard soundcard on a computer would have to be for a separate headphone output. Also available bundled with a microphone and headphones for $200.                                                                                       |
...
| [Roland Rubix24](https://www.roland.com/us/products/rubix24/)                                                                             | $200               | 4            | 2           | Good balance of sound quality and price with the recommended number of channels. Balanced outputs can plug directly into a PA system without needing an external mixer. Can be used with live inputs (like microphones for radio broadcasting), but only has 2 input channels, which could only support one timecode vinyl deck. Has microphone inputs with an analog compressor that can be used with direct monitoring. |
