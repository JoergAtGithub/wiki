...
tend to be easier to set up. Mixxx can use multiple sound cards at the
same time, but this increases latency and there may be crackling on one
sound card, so a single sound card with at least 4 output channels is
