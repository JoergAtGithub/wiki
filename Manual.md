...
5.  [Configuration](manual#configuration)
    ...
    2.  [External Mixer Mode](manual#external_mixer_mode)
    3.  [Ladspa Effects](manual#ladspa_effects)
    4.  [Latency](manual#latency)
    5.  [Samplerates](manual#samplerates)
    6.  [Sound APIs](manual#sound_apis)
    7.  [Vinyl Control](manual#vinyl_control)
...
<br />
Use TAP to set the BPM if Mixxx cannot detect it, or has calculated it incorrectly. Press TAP a few times to coincide with the beat of the playing track, and the BPM will be calculated from the average of the last four taps. REPEAT will make the track begin playing again once it reaches the end. The track will also repeat in reverse if 'rewound' through the beginning.</td>
</tr>
</tbody>
</table>
...
As of Mixxx 1.9, if you are using an external (hardware) mixer, you can
route Deck 1 and Deck 2 to separate channels. This requires either two
soundcards or a soundcard with more than 2 channel output. To do this,
...
[[/media/mixxx-jackd-jackrack-setup-screenshot_3_small.jpg|<http://mixxx.org/wiki/lib/exe/fetch.php/mixxx-jackd-jackrack-setup-screenshot_3.gif>]]
