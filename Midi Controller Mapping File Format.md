...
|  | \[Channel*N*\] |  | loop\_double\[20\]            |  | binary                 |  | Doubles the current loop's length by moving the end marker.                                                           |  | Loop length doubles on waveform                                                         |  |
|  | \[Channel*N*\] |  | loop\_halve\[21\]             |  | binary                 |  | Halves the current loop's length by moving the end marker. Player immediately loops if past the new endpoint.         |  | Loop length halves on waveform                                                          |  |
|  | \[Channel*N*\] |  | loop\_scale\[22\]             |  | 0.0 - infinity         |  | Scale the loop length by the value scale is set to by moving the end marker.                                          |  | Loop length is scaled by given amount on waveform.                                      |  |
|  | \[Channel*N*\] |  | beatloop\[23\]                |  | 0.0625 - 128           |  | Setup a loop over the set number of beats.                                                                            |  | A loop is shown over the set number of beats.                                           |  |
|  | \[Channel*N*\] |  | beatloop\_(X)\[24\]           |  | 0.0 / 1.0 (PushButton) |  | Setup a loop over the set number of beats. A control exists for X = 0.0625, 0.125, 0.25, 0.5, 1, 2, 4, 8, 16, 32, 64  |  | A loop is shown over the set number of beats.                                           |  |
|  | \[Channel*N*\] |  | quantize\[25\]                |  | 0.0 / 1.0 (PushButton) |  | Aligns Hotcues, CUE and Loop In & Out to the next beat starting from the current position.                            |  | Hotcues, CUE and Loop In & Out are aligned to the next beat from the previous position. |  |
|  | \[Channel*N*\] |  | quantize\_beat\[26\]          |  | 0.0 - infinity         |  | Holds the frame offset position of the next beat.                                                                     |  | Is used internally by CueControl (CUEs & Hotcues) and LoopingControl for quantization.  |  |
