...
turn on when sent value 0x00, your init and shutdown functions could be:
    ...
    MyController.shutdown = function() {
       for (i = 1; i <= 40; i++) {
            [[#Sending messages to the controller|midi.sendShortMsg(
                0x90,
                i,
                0x00
            )]]
...
map it to a function like the playButton function in the script below
that starts with 'group = MyController.deck\[group\]'.
...
MyController.playButton = function (channel, control, value, status, group) {
    ...
    if (value) {
        engine.setValue(group, 'play', ! (engine.getValue(MyController.deck[group], 'play') )
    }
}
```
