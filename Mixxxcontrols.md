...
| `PeakIndicator`                      | binary             | Indicates when the signal is clipping (too loud for the hardware and is being distorted) (composite)                                                                           | Clip light (mono)                |
| `PeakIndicatorL`                      | binary             | Indicates when the signal is clipping (too loud for the hardware and is being distorted) for the left channel                                                                           | Clip light (left)                |
| `PeakIndicatorR`                      | binary             | Indicates when the signal is clipping (too loud for the hardware and is being distorted) for the right channel                                                                          | Clip light (right)               |
...
| `quantize_beat`\[124\]                   | deprecated                    |  Used internally by CueControl (CUEs & Hotcues) and LoopingControl for quantization.                                                                                             | |
