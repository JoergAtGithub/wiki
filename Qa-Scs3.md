...
Return to the original pitch (tonic) by pressing B12 (under the pitch
slider.)
...
(lit up dim red for visibility.) The TRIG button will be red in bank 1,
...
(Mixxx 1.8 internally supports 31 hot cues and the SCS.3d mapping has
been adjusted to match. That means the center 4 buttons and the lower
right one in the third bank (black) are inoperative.)
...
You are in this mode only **while holding down the Deck button**, unless
the controller is in single-deck mode:
...
*Make sure changing the customization variables has the described
effect.*
